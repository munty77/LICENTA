...
function SearchData(){
    let searchProd = document.getElementById("searchText").value;
    const q = query(ref(db, "Produse"),orderByChild("Nume"),startAt(searchProd),endAt(searchProd));
    // const q = query(ref(db, "Produse"),orderByChild("Nume"),startAt(searchProd));//,endAt(searchProd + "\uf8ff"));
    // const q = query(ref(db, "Produse"),orderByChild("Nume"),startAt(equalsIgnoreCase(searchProd)));
    console.log("Numele cautat - - ", searchProd);
    get(q)
    .then((snapshot) => {
        var prod = [];
        snapshot.forEach(childSnapshot => {
...
btnOrders.addEventListener('click', () => {
    ...
    get(child(dbRef, "Conturi/")).then((snapshot) => {
        snapshot.forEach(childObj => {
            ...
            console.log("All data - - ", childObj.val());
            console.log("Nume Client - - ", childObj.val().Nume);
            console.log("Mail Client - - ", childObj.val().Mail);
            console.log("Nr. Telefon Client - - ", childObj.val().Telefon);
            console.log("Adresa Client - - ", childObj.val().Adresa);
            console.log("Oras Client - - ", childObj.val().Oras);
            console.log("Judet Client - - ", childObj.val().Judet);
            console.log("Cod Postal Client - - ", childObj.val().Cod_postal);
            console.log("Cos - - - ", childObj.val().CosCumparaturi );

            get(child(dbRef, "Conturi/" + childObj.key + "/CosCumparaturi/")).then((snapshot) => {
                snapshot.forEach(childCos => {
                    console.log("key user - - - ", childObj.key, " Key prod - - - ", childCos.key);
                    console.log("obj Color - - - ", childCos.val());

                    get(child(dbRef, "Conturi/" + childObj.key + "/CosCumparaturi/" + childCos.key)).then((snapshot) => {
                        snapshot.forEach(childColors => {
                            console.log("key color - - -", childColors.key);
                            console.log("Nume Prod - - - ", childColors.val().Nume);
                        })
                    })
                })
            })
